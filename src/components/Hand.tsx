...
export default function Hand({ hand, selectedCardId, onSelectCard, isMyTurn }: Props) {
  ...
  return (
    <Reorder.Group 
      ...
      className="flex gap-2 sm:gap-4 overflow-x-auto pb-4 sm:pb-6 pt-1 sm:pt-2 px-1 sm:px-2 scrollbar-none custom-scrollbar"
    >
      {orderedHand.map((card, index) => {
        ...
        return (
          <Reorder.Item
            key={card.id}
            value={card}
            dragListener={isMyTurn}
            className={`group relative shrink-0 transition-all duration-300 ${
              isSelected ? 'z-30' : 'z-10'
            } ${!isMyTurn ? 'cursor-not-allowed grayscale-[0.5] opacity-60' : 'cursor-grab active:cursor-grabbing'}`}
          >
            <motion.button
              type="button"
