  ...
  return (
    ...
    >
      <motion.button
        ...
        className={`w-[5.2rem] sm:w-[7.5rem] ${!isMyTurn ? 'cursor-not-allowed grayscale-[0.5] opacity-60' : 'cursor-pointer'}`}
      >
        {/* Card Body */}
        <div className={`relative h-[7.4rem] w-full sm:h-[11rem] bg-[#fdfdfd] p-1.5 sm:p-3 shadow-2xl transition-all duration-500 overflow-hidden ${styles.border} ${
          isSelected 
          ...
          {(isTwoEyed || isOneEyed) && (
            <div className="absolute inset-x-0 bottom-1 sm:top-1/2 sm:-translate-y-1/2 px-1 z-20">
              <div className="bg-slate-900 border border-[#c5a059]/50 text-[#c5a059] text-[7px] sm:text-[8px] font-black uppercase tracking-widest py-[3px] sm:py-1 px-1.5 sm:px-2 rounded-full text-center shadow-lg">
